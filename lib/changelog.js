...
async function getSubmoduleChangelog(submodule) {
  ...
  if (firstCommit === lastCommit) {
    ...
    compare = `[${lastCommit.commit.short}...${firstCommit.commit.short}](https://github.com/${gitURLInfo.owner}/${gitURLInfo.name}/compare/${lastCommit.commit.short}...${firstCommit.commit.short})`;
  }
  return `${title}\n\n${compare}\n\n${changelog}`;
}
...
function changelog(url) {
  ...
  return fs.promises
    ...
    .then(() => {
      return sh('git submodule update');
    })
    .then(() => {
